...
#define CTIMER_CLK_FREQ 6000000 // 6 MHz
...
void ctimer_match0_callback(uint32_t flags)
{
	...
	uint16_t adc_val = (mLpadc_resultConfigStruct.convValue >> 3);
...
//    DAC_SetData(DAC1, loadVal);
    ...
    sine_index++;
    if (sine_index >= SINE_POINTS) sine_index = 0;
}
